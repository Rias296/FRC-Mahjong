...
export function ClaimActionBar({ offer, onAction, pending }: ClaimActionBarProps): React.JSX.Element | null {
  ...
  return (
    <div
      className="panel fixed right-4 bottom-4 left-4 z-30 flex flex-wrap items-center justify-center gap-2 rounded-xl p-3 shadow-lg sm:right-auto sm:left-1/2 sm:w-auto sm:-translate-x-1/2"
      role="group"
      ...
      {offer.map((claim) => {
        ...
        return (
          <Button
            ...
            disabled={pending}
            onClick={() => onAction(claim)}
            className={cn(TOUCH_TARGET_CLASS, 'border-accent/60 text-accent hover:bg-accent/10')}
          >
            {actionLabel(claim)}
          </Button>
        );
